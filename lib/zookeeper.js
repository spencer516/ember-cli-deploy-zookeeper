...
var REVISION_KEY = 'revisions';

module.exports = CoreObject.extend({
  init: function(options, lib) {
    var zkOptions = options;
    var zkLib = lib;
    ...
    this._client = zkLib.create ? zkLib.create(zkOptions) : new zkLib(zkOptions);
  ...
  activeRevision: function(keyPrefix) {
    ...
    return this._client.get(path).then(function(result) {
      return result.data;
    });
  },
  ...
  _listRevisions: function(keyPrefix) {
    var path = makePath(keyPrefix, REVISION_KEY);
    return this._client.getChildren(path).then(function(res) {
      return res.children;
    });
  },
  _createMissingParentPaths: function(parts) {
    ...
    return parts.reduce(function(promise, key, index) {
      ...
      return promise.then(function() {
        return setOrCreateIfNotExists(client, key);
      });
  ...
  _upload: function(zkKey, value) {
    ...
    return setOrCreateIfNotExists(client, zkKey, value);
  },

  _updateRecentUploadsList: function(keyPrefix, revisionKey) {
    var client = this._client;
    var timeAdded = new Date().getTime();
    var listKey = makePath(keyPrefix, REVISION_KEY, revisionKey);

    return setOrCreateIfNotExists(client, listKey, timeAdded);
  },
  ...
  _trimRecentUploadsList: function(keyPrefix, maxEntries) {
    ...
    return this.fetchRevisions(keyPrefix).then(function(results) {
      ...
      return Promise.all(results.filter(function(revision) {
        ...
        var path = makePath(keyPrefix, REVISION_KEY, revision.revision);
        return Promise.hash({
  ...
  _activateRevisionKey: function(keyPrefix, revision) {
    var path = makePath(keyPrefix);
    return setOrCreateIfNotExists(this._client, path, revision);
  }
});

function setOrCreateIfNotExists(client, key, value) {
